...
def poll_side_a() -> dict[str, Any]:
    ...
    Uses ComponentRegistry.get_orchestrator() to get the SHARED singleton,
    not a fresh empty instance (fixes BRUTAL_DEEP_SCAN FINDING #2).
    ...
    # ── Execution orchestrator state (via ComponentRegistry) ──
    try:
        orchestrator = get_registry().get_orchestrator()
        ...
        log.warning("Side A orchestrator poll failed — using defaults", exc_info=True)

    # ── Blocked action journal (via ComponentRegistry) ──
    try:
        journal = get_registry().get_execution_registry().get_blocked_journal()
        ...
        log.debug("Blocked action journal poll failed", exc_info=True)
    ...
    try:
        ...
        log.debug("Execution logs poll failed", exc_info=True)
